...
int interpol_search(int arr[], int lb, int ub, int x)
{
    ...
    while ((arr[lb] != arr[ub]) && (x <= arr[ub]) && (x >= arr[lb])) {
        ...
        if (arr[pos] == x){
        	printf("Search Element :%d Step Count:%d\n",x,step_count);
        	return pos;
        }
